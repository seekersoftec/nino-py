#
# Information Gathering
#
#
import random
#
from modules.hibp_pastes import email_pastes
from modules.domaineye import email2domains
from modules.gravatar import email2gravatar
from modules.linkedin import email2linkedin
from modules.aboutme import email2aboutme
from modules.myspace import email2myspace
from modules.github import email2github
from modules.skype import email2skype
from modules.avast import email2breachedaccts
from modules.twitter import twitter_search
from modules.darksearch import dark_search
from modules.googledork import google_search
#
from modules.requests_lib import *
#
from modules.useragents import *
#


#
class EmailChecker:
    def __init__(self):
        self.BASE_URL = 'https://emailrep.io'
        self.async_req = AsyncRequests()

    #
    def get_api_endpoint(self, email_address=''):
        return self.BASE_URL+"/{0}".format(email_address)

    #
    def convert_response_obj(self, obj):
        #
        if obj.status_code == 200:
            return obj.json()
        else:
            return {}

    #
    def verify(self, email_address=''):
        _tor_req = TorRequests()
        #
        try:
            get_res = _tor_req.get(self.get_api_endpoint(email_address))
        except KeyboardInterrupt:
            raise SystemExit

        return self.convert_response_obj(get_res)

    #

    def check(self, email):
        email_info = {"email": email, "exists": False,
                      "emailrep_limit_reached": False}
        headers = {'User-Agent': random.choice(USER_AGENTS)}

        accounts = []
        sources = []
        #
        reqs = [
            self.async_req.request(
                method='GET', url='https://myspace.com/search/people?q=' + email),

            self.async_req.request(method='GET', url='https://api.github.com/search/users?q=' +
                                   email + '+in:email'),

            self.async_req.request(method='GET',
                                   url='https://darksearch.io/api/search?query="' + email + '"'),

            self.async_req.request(method='GET',
                                   url='https://haveibeenpwned.com/api/v2/pasteaccount/' + email, headers=headers),

            self.async_req.request(method='POST', url='https://digibody.avast.com/v1/web/leaks',
                                   json={'email': email})
        ]

        response = self.async_req.map(reqs)

        #
        _verify_email = self.verify(email)
        #
        # emailRep.io
        if ((isinstance(_verify_email, dict)) and (len(_verify_email) != 0)):
            data = _verify_email

            if (data["details"]["deliverable"] == True):
                email_info["exists"] = True

            if (data["details"]["last_seen"] != "never"):
                email_info["exists"] = True

            if (data["details"]["profiles"] != []):
                email_info["exists"] = True
                email_info["profiles"] = data["details"]["profiles"]
                if "gravatar" in email_info["profiles"]:
                    gravatar = email2gravatar(email)
                    if gravatar != []:
                        accounts.extend(gravatar)
                if "aboutme" in email_info["profiles"]:
                    aboutme = email2aboutme(email)
                    if aboutme != []:
                        accounts.extend(aboutme)
                if "linkedin" in email_info["profiles"]:
                    accounts.append(email2linkedin(email))

            #
            # myspace
            myspace = email2myspace(email, response[0])
            if (myspace != []):
                email_info["exists"] = True
                accounts.extend(myspace)

            # github
            github = email2github(email, response[1])
            if (github != ""):
                email_info["exists"] = True
                accounts.append(github)

            # dark search
            darksearch_sources = dark_search(email, response[2])
            if (darksearch_sources != []):
                email_info["exists"] = True
                sources.extend(darksearch_sources)

            # haveibeenpwned
            pastes = email_pastes(email, response[3])
            if (pastes != []):
                email_info["exists"] = True
                email_info["pastes"] = pastes

            breached_accts = email2breachedaccts(email, response[4])
            if (breached_accts["accounts"] != []):
                email_info["exists"] = True
                accounts.extend(breached_accts["accounts"])

            if (breached_accts["breaches"] != []):
                email_info["exists"] = True
                email_info["breaches"] = breached_accts["breaches"]

            if (email_info["exists"] == True):

                skype = email2skype(email)
                if (skype != []):
                    accounts.extend(skype)

                google_sources = google_search(email)
                if (google_sources != []):
                    sources.extend(google_sources)

                twitter_sources = twitter_search(email)
                if (twitter_sources != []):
                    sources.extend(twitter_sources)

                domains_registered = email2domains(email)
                if (domains_registered != []):
                    email_info["domains_registered"] = domains_registered

            if accounts != []:
                email_info["accounts"] = accounts
            if sources != []:
                email_info["sources"] = sources

        else:
            email_info["emailrep_limit_reached"] = True

        return email_info


#
#
#
# _ch = EmailChecker().check('safety4ualways@gmail.com')
# _ch = EmailChecker().verify('seekersoftec@gmail.com')
# print(_ch)

#

#
