...
def email2skype(email):
    ...
    headers = {'Referer': 'https://webresolver.nl/tools/email_to_skype',
               'X-Requested-With': 'XMLHttpRequest'}

    try:
        response = requests.get("https://webresolver.nl/tools/email_to_skype")
        soup = BeautifulSoup(response.content, 'html.parser')
        token = str(soup.body.script).split(
            'WebResolverSecurityCode=')[1].split('; expires=')[0]
        ...
        print("[=]Warning:Something went wrong while attempting to scrap webresolver.nl")
        return []
